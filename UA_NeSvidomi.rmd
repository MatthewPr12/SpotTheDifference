...
P-value represents the probability to obtain such extreme difference in means, supposing $\mu_0$ is true. Here p-value is super small, which means that we reject our $H_0$, and the average number of Ukrainian songs before the full scale invasion is not equal to the number of Ukrainian songs after the full scale invasion $\mu0 \lt \mu_1$. It can even be seen from calculated means whcih are $\approx7.5$ and $\approx 52.7$ respectively. Or it is also obvious from the plotted data.
...
$$T = \frac{\bar{x_0} - \bar{x_1}}{\sqrt{\frac{s_0^2}{n_0} + \frac{s_1^2}{n_1}}}$$
Then, we will reject our $H_0$ if $|T|\gt t_{1-\alpha/2, \nu}$ where $\nu$ represents degrees of freedom
```{r}
t.test(as.numeric(unlist(EN_quant_before)), as.numeric(unlist(EN_quant_after)), conf.int = T, conf.level = 0.95)
```

As we can see from the results of the t-test, p-value is much bigger than 0.05($\alpha$)
so we do not reject the $H_0$
